...
KFbxCamera* GetCurrentCamera(KFbxScene* pScene, 
                             ...
                             KArrayTemplate<KFbxNode*>& pCameraArray)
{
    KFbxGlobalCameraSettings& lGlobalCameraSettings = pScene->GetGlobalCameraSettings();
    KString lCurrentCameraName = lGlobalCameraSettings.GetDefaultCamera();
    ...
    {
        char* takeName = pScene->GetRootNode()->GetCurrentTakeNodeName();
        KFbxCameraSwitcher* lCameraSwitcher = pScene->GetGlobalCameraSettings().GetCameraSwitcher();
        KFCurve* lFCurve = lCameraSwitcher->CameraIndex.GetKFCurve(NULL, takeName);
...
void GetCameraAnimatedParameters(KFbxNode* pNode, KTime& pTime)
{
    ...
    if (lCamera->GetApertureMode() == KFbxCamera::eHORIZONTAL || 
        lCamera->GetApertureMode() == KFbxCamera::eVERTICAL) 
    {
        fc = lCamera->FieldOfView.GetKFCurve(NULL, takeName);
        if (fc)
    ...
    else if (lCamera->GetApertureMode() == KFbxCamera::eHORIZONTAL_AND_VERTICAL)
    {
        ...
        fc = lCamera->FieldOfViewX.GetKFCurve(NULL, takeName);
        if (fc)
            lFieldOfViewX = fc->Evaluate(pTime);

        fc = lCamera->FieldOfViewY.GetKFCurve(NULL, takeName);
        if (fc)
        ...
        //double lOpticalCenterX = pNode->GetCurrentTakeNode()->GetCameraOpticalCenterX()->Evaluate(pTime);
        //double lOpticalCenterY = pNode->GetCurrentTakeNode()->GetCameraOpticalCenterY()->Evaluate(pTime);
    }
    else if (lCamera->GetApertureMode() == KFbxCamera::eFOCAL_LENGTH)
    {
        double lFocalLength = lCamera->FocalLength.Get();
        fc = lCamera->FocalLength.GetKFCurve(NULL, takeName);
