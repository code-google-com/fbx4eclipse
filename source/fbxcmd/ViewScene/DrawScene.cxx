...
// Draw the scene at a given time for the current take.
...
// will be drawn at the given time for the current take.
                             ...
                             KFbxVector4* pVertexArray)
{
    ...
    for (i = 0; i < lShapeCount; i++)
    {
        ...
        KFCurve* lFCurve = pMesh->GetShapeChannel(i);
        if (!lFCurve) continue;
                ...
                KFbxXMatrix& pGlobalPosition)
{
    ...
    if (cam)
    {
        ...
        KFCurve* fc = cam->Roll.GetKFCurve(NULL, takeName);
               ...
               KFbxXMatrix& pGlobalPosition)
{
    ...
    fc = lLight->Color.GetKFCurve(KFCURVENODE_COLOR_RED, takeName);
    if (fc) lColor.mRed = fc->Evaluate(pTime);
    fc = lLight->Color.GetKFCurve(KFCURVENODE_COLOR_GREEN, takeName);
    if (fc) lColor.mGreen = fc->Evaluate(pTime);
    fc = lLight->Color.GetKFCurve(KFCURVENODE_COLOR_BLUE, takeName);
    ...
    {
        fc = lLight->ConeAngle.GetKFCurve(NULL, takeName);
