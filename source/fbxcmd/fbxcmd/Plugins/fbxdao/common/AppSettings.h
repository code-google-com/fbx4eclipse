...
public:
	...
	template<typename T>
	inline bool TryGetSetting(LPCTSTR setting, T& v){
		IniSettingCollection::iterator itr = Environment.find(setting);
		if (itr != Environment.end()){
			_tstringstream sstr((*itr).second.val);
			sstr >> v;
	...
	template<>
	inline bool TryGetSetting(LPCTSTR setting, _tstring& value){
		IniSettingCollection::iterator itr = Environment.find(setting);
		if (itr != Environment.end()) {
	...
	template<>
	inline bool TryGetSetting(LPCTSTR setting, bool& value){
		int ival;
