				...
				)
{
			...
			case DECLUSAGE_POSITION:		
				{
					...
					for (int i = 0; i<nverts; ++i) {
						...
						log.PrintF("v %7.4f %7.4f %7.4f\n", vert[0], vert[1], vert[2]);
					}
					log.PrintF("# %d vertices\n", nverts);
			...
			case DECLUSAGE_NORMAL:			
				{
					...
					for (int i = 0; i<nverts; ++i) {
						...
						Vector4f& vert = verts[i];
						AssignValue( vert, DECLValue(decl->get_dataType(), vertStream.dataptr()) );
						log.PrintF("vn %7.4f %7.4f %7.4f\n", vert[0], vert[1], vert[2]);
					}
					log.PrintF("# %d vertex normals\n", nverts);
				}
				break;
	...
	int start = chnk->get_Startindex() + 1;
	...
	for (int i=0; i<nfaces; ++i) {
		...
		log.PrintF("f %hu//%hu %hu//%hu %hu//%hu\n"
			, t[0] + startIdx, t[0]+ startIdx
			, t[1] + startIdx, t[1]+ startIdx
			, t[2] + startIdx, t[2]+ startIdx
			);
...
void ImportMesh(DAODumpStream out, MESHRef root)
{
	DAOMemStream vertStream;
	DAOMemStream idxStream;
	...
	if ( DynamicCast<CHNK>(root->get_chunks(), chunks) ) {
		for (DAOArray<CHNKPtr>::iterator itr = chunks.begin(); itr != chunks.end(); ++itr) {
			CHNKRef chnk(*itr);
			ImportMESH(out, chnk, idxStream, vertStream, startIdx);
		}
	}
}
...
static void HelpString(Cmd::HelpType type){
	switch (type)
	{
	case Cmd::htShort: cout << "DumpObj - Dump MESH Mesh information to console"; break;
	case Cmd::htLong:  
		{
...
static bool ExecuteCmd(Program &prog)
{
	...
	if ( (fstats.st_mode & _S_IFDIR) != _S_IFDIR ) // file not directory
	{
		...
		DAODumpStream out("c:\\tmp\\fdsa.obj", false);

		ImportMesh( out, MESHRef(root) );
...
REGISTER_COMMAND(DumpObj, HelpString, ExecuteCmd);
